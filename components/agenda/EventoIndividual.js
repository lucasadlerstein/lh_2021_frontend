...
const EventoIndividual = ({t, evento}) => {
    ...
    return (
        <Item>
            ...
            <Row className="evento-individual">
                ...
                } <span>{`(${codigoPais})`}</span></Hora>
                    {
                        ...
                        (
                            (evento.orador_imagen === null || evento.orador_imagen === '') ? null : (
                                <FotoOrador>
                                    <img src={`https://api.latamhospitals.com/static/${evento.orador_imagen}`} alt={evento.orador_nombre + ' ' + evento.orador_apellido} />
                                    {
                                        (evento.dos_orador_imagen === null || evento.dos_orador_imagen === '') ? null : (
                                            <img src={`https://api.latamhospitals.com/static/${evento.dos_orador_imagen}`} className="m-3" alt={evento.dos_orador_nombre + ' ' + evento.dos_orador_apellido} />
                                        )
                                    }
                                    {
                                        (evento.tres_orador_imagen === null || evento.tres_orador_imagen === '') ? null : (
                                            <img src={`https://api.latamhospitals.com/static/${evento.tres_orador_imagen}`} alt={evento.tres_orador_nombre + ' ' + evento.tres_orador_apellido} />
                                        )
                                    }
                                    {
                                        (evento.cuatro_orador_imagen === null || evento.cuatro_orador_imagen === '') ? null : (
                                            <img src={`https://api.latamhospitals.com/static/${evento.cuatro_orador_imagen}`} className="m-3" alt={evento.cuatro_orador_nombre + ' ' + evento.cuatro_orador_apellido} />
                                        )
                                    }
                                </FotoOrador>
                            )
                            ...
                            <Logo src={`https://api.latamhospitals.com/static/${evento.logo_empresa}`} alt={evento.nombre_empresa} />
                        )
                    }
                </Col>
                ...
                <Col xs={12} lg={3}>
                    {/* <BotonInscripcion onClick={() => setInscribirme(true)} className="btn-lh btn-sec text-white bor-rad-5 my-4 d-block m-0">{t('Charla.Botones.Inscribirme')}</BotonInscripcion> */}
                    {/* <Link href={(Number(evento.categoria) === 1) ? `/mastertalk/${evento.slug}` : `/conferencia/${evento.slug}`}>
                        <a className="btn-lh btn-prim text-white bor-rad-5 my-4 d-block mr-0" target="_blank">{t('Charla.Botones.MasInfo')}</a>
                    </Link> */}
                    <Link href={(Number(evento.categoria) === 1) ? `/mastertalk/${evento.slug}` : `/conferencia/${evento.slug}`}>
                        <a className="btn-lh btn-prim text-white bor-rad-5 my-4 d-block mr-0" target="_blank">Ver grabación</a>
                    </Link>
                </Col>
            </Row>
        </Item>
...
export default withTranslation('charla')(EventoIndividual);